...
let make: 'a 'b. ('a => option('b), ('b, 'a) => 'a) => t('a, 'b) =
  (getOption, set) => {getOption, set};
...
let compose: 'a 'b 'c. (t('a, 'b), t('b, 'c)) => t('a, 'c) =
  (
    ...
    make(
      ...
      (c, a) =>
        outerGetOption(a)
        ->Option.map(x => c->innerSet(x)->outerSet(a))
        ->Option.getWithDefault(a),
    );

/** Map over an Optional value */
let modify: 'a 'b. (t('a, 'b), 'b => 'b, 'a) => option('a) =
  ({getOption, set}, f, a) => a->getOption->Option.map(x => x->f->set(a));

/** Map over an Optional value, returns a default value if None */
let modifyWithDefault: 'a 'b. (t('a, 'b), 'b => 'b, 'a) => 'a =
  (optional, f, x) => modify(optional, f, x)->Option.getWithDefault(x);

/** Compose a Lens and an Optional */
...
/** Makes an Optional from a Lens */
let fromLens: 'a 'b. Lens.t('a, 'b) => t('a, 'b) =
  lens => make(a => Some(lens.get(a)), lens.set);
