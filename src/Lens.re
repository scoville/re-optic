...
let compose: 'a 'b 'c. (t('a, 'b), t('b, 'c)) => t('a, 'c) =
  ({get: outerGet, set: outerSet}, {get: innerGet, set: innerSet}) =>
    make(
      x => x->outerGet->innerGet,
      (c, a) => a->outerGet->innerSet(c, _)->outerSet(a),
    );

/** Map over a Lenss value */
let modify: 'a 'b. (t('a, 'b), 'b => 'b, 'a) => 'a =
  ({get, set}, f, x) => x->get->f->set(x);

/** Makes a Lens from an Iso */
