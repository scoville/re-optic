@@ocaml.doc("Some common optic")

let id: Iso.t<'a, 'a> = {get: x => x, reverseGet: x => x}

let stringBool: Prism.t<string, bool> = Prism.make(bool_of_string_opt, string_of_bool)

let stringInt: Prism.t<string, int> = Prism.make(int_of_string_opt, string_of_int)
...
let variant:
  type a. module(VARIANT with type t = a) => Prism.t<string, a> =
  (module(Variant: VARIANT with type t = a)) => Prism.make(Variant.tFromJs, Variant.tToJs)

module Array = {
  let index: 'a. int => Optional.t<array<'a>, 'a> = i =>
    Optional.make(
      x => x->Array.get(i),
      (c, x) => Array.mapWithIndex(x, (i', entry) => i != i' ? entry : c),
    )

  let indexExn: 'a. int => Lens.t<array<'a>, 'a> = i =>
    Lens.make(
